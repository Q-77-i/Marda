...
> 版本 v1.0 ｜ 2026-09-15 ｜ 状态：待评审 ｜ 上游：docs/规划报告.md（已确认）｜ 下游：SPEC
...
| 编号 | 需求 | 说明 |
| --- | --- | --- |
| FR-01 | 创建面试 | 选择岗位方向（阶段 1：Agent/AI 工程师）、题量（默认 10 技术题 + 1 场景题，可选 5/10/15）、开始按钮；单用户无登录 |
...
| 编号 | 需求 | 优先级 | 说明 |
| --- | --- | --- | --- |
| FR-10 | 语料解析入库 | P0 | 解析三格式个人题库：md（面次→厂商→主题→题目层级）、xmind（zip 解包）、PDF（pypdf/pdfplumber 按标题切分），产出结构化题目 JSON（§4.6 字段） |
...
| FR-17 | 报告页 | P0 | 逐题点评（题目/回答摘要/得分/追问过程/与参考答案关键点对比）+ 短板定位（得分最低的 2-3 个知识域）+ 下次面试建议 |
...
| TECH_BASE 技术问答 | 按能力模型出 N 道技术题（默认 10），每题走"出题→作答→评分→追问决策"循环 | 计 |
...
| 面试页 | 聊天流（打字机流式）、当前阶段与进度指示（如 "技术问答 7/10"）、主动结束按钮、断线恢复提示 | P0 |
| 报告页 | 雷达图（Recharts）、知识域分布、逐题点评、短板、下次建议、PDF 导出按钮（P1） | P0/P1 |
...
1. 题库 ≥100 题结构化入库（来源：个人题库三格式解析），字段完整率 ≥95%，解析脚本有单测；
2. 默认配置（10 技术 + 1 场景）完整跑通一场面试，全程流式，无中断；
...
1. 默认题量：**10 技术题 + 1 场景题**（可选 5/10/15）；
2. 评分制：**1-5 分整数**；
